...
Deletes existing claims from a search engine index.
...
from pipelines.search_index_delete.pipeline import run_pipeline

PIPELINE_NAME = "search-index-delete"
...
def main(argv=None):
    ...
    parser.add_argument(
        ...
        required=True,
        help="Search index ID to delete from",
    )
    parser.add_argument(
        "--db_env",
        dest="db_env",
    ...
    if search_env == SearchEnv.PROD:
        ...
        return
    if (
        search_env != SearchEnv.PROD_INGEST
        and search_env != SearchEnv.DEV
        and search_env != SearchEnv.STAGING
    ):
