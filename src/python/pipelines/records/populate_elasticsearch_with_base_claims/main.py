"""
Iterates over claim feature records and writes base claims to a newly created
Elasticsearch index.
"""
import argparse
import logging
...
from pipelines.records.populate_elasticsearch_with_base_claims.pipeline import run_pipeline

PIPELINE_NAME = "populate-elasticsearch-with-base-claims"
...
def main(argv=None):
    ...
    if db_env != DbEnv.PROD_INGEST and db_env != DbEnv.DEV:
        logging.error(f"You can only ingest into db prod-ingest or dev, not {db_env}.")
        return
